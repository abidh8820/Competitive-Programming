...
void fft(vector<cd>& a, bool invert) {
    // keeps the points after converting in the same vector
    int n = a.size();
    if (n == 1) return;
...
vector<int> multiply(vector<int> const& a, vector<int> const& b) {
    // takes two intger vectors and typecast them into two complex vectors
    ...
    // converting coefficients into points
    fft(fa, false), fft(fb, false);

    // multiplying the points
    for (int i = 0; i < n; i++) fa[i] *= fb[i];

    // converting back points to coefficients
    fft(fa, true);
    vector<int> result(n);
...
// pairfft is much faster compared to the one imlemented above
/**
