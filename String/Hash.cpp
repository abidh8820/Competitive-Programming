...
void dfs(int u, int p, int depth, PLL forhash, PLL revhash) {
    forhash = append(forhash, card[u]);
    revhash = prepend(revhash, depth, card[u]);
    cnt += (forhash == revhash);
    for (int v : adj[u]) {
        if (v ^ p) {
            dfs(v, u, depth + 1, forhash, revhash);
        }
    }
}
...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; i++) card[i + 1] = s[i];
        for (int i = 1; i < n; i++) {
            int u, v;
            cin >> u >> v;
            adj[u].push_back(v);
